...
describe('Test de RUTAS', () => {
    ...
    describe('GET /rickandmorty/character/:id', () => {
        ...
        it('Si hay un error responde con status: 500', async () => {
            await agent.get('/rickandmorty/character/111111111').expect(500);
        });
    });

    describe('GET /rickandmorty/login', () => {
        it('Retorna un objeto con la propiedad access en true si recibe credenciales correctas', async () => {
            const response = await agent.get('/rickandmorty/login?email=example@gmail.com&password=password12');
            expect(response.body.access).toBe(true);
        });
        it('Retorna un objeto con la propiedad access en true si recibe credenciales incorrectas', async () => {
            const response = await agent.get('/rickandmorty/login?email=example@gmail.com&password=password12123');
